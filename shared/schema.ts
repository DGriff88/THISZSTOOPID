...
// Algorithmic Trading Strategies (ALGOTRADER BRAINBOX)
export const algorithmicStrategies = pgTable("algorithmic_strategies", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  userId: varchar("user_id").notNull().references(() => users.id),
  strategyName: text("strategy_name").notNull(),
  description: text("description"),
  // Scanner configuration
  catalystSources: text("catalyst_sources").array().notNull(), // ['SEC', 'FCC', 'DOJ', 'PressReleases', 'UOA', 'PremarketRVOL']
  scannerFilters: jsonb("scanner_filters").notNull(), // {rvolMin, priceMin, spreadMax, avoid}
  // Risk management
  dailyLossLimit: decimal("daily_loss_limit", { precision: 10, scale: 2 }).notNull(),
  walkRuleLosses: integer("walk_rule_losses").notNull(),
  capitalHalfRoll: boolean("capital_half_roll").notNull().default(true),
  maxRiskPerTrade: decimal("max_risk_per_trade", { precision: 10, scale: 2 }).notNull(),
  stopToBreakeven: boolean("stop_to_breakeven").notNull().default(true),
  riskRewardMin: decimal("risk_reward_min", { precision: 3, scale: 1 }).notNull(),
  // Entry rules
  entryRules: jsonb("entry_rules").notNull(), // {emaTrend, oversoldBounce, momentumPop}
  // Compliance
  complianceRules: jsonb("compliance_rules").notNull(), // {ruleOne, noStrayLegs, dailyLog}
  // Objectives
  dailyTarget: decimal("daily_target", { precision: 10, scale: 2 }).notNull(),
  weeklyBaseline: decimal("weekly_baseline", { precision: 10, scale: 2 }).notNull(),
  weeklyStretch: decimal("weekly_stretch", { precision: 10, scale: 2 }).notNull(),
  // Status
  isActive: boolean("is_active").notNull().default(false),
  isPaperTrading: boolean("is_paper_trading").notNull().default(true),
  createdAt: timestamp("created_at").notNull().defaultNow(),
  ...
  userIdIdx: index("algorithmic_strategies_user_id_idx").on(table.userId),
  userIdActiveIdx: index("algorithmic_strategies_user_id_active_idx").on(table.userId, table.isActive),
  strategyNameIdx: index("algorithmic_strategies_name_idx").on(table.strategyName),
}));

// Strategy Stock Picks - Generated by algorithmic strategies for stock selection
export const strategyStockPicks = pgTable("strategy_stock_picks", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  strategyId: varchar("strategy_id").notNull().references(() => algorithmicStrategies.id),
  symbol: text("symbol").notNull(),
  reason: text("reason").notNull(), // Why this stock was selected
  entrySignal: text("entry_signal").notNull(), // emaTrend, oversoldBounce, momentumPop
  scannerScore: decimal("scanner_score", { precision: 5, scale: 2 }), // 0-100 confidence score
  rvol: decimal("rvol", { precision: 5, scale: 2 }), // Relative volume
  price: decimal("price", { precision: 10, scale: 2 }).notNull(),
  rsi: decimal("rsi", { precision: 5, scale: 2 }), // RSI indicator
  bollingerPosition: text("bollinger_position"), // 'upper', 'lower', 'middle'
  macdCurl: boolean("macd_curl"), // MACD curl signal
  breakoutRetest: boolean("breakout_retest"), // Breakout retest pattern
  status: text("status").notNull().default('active'), // 'active', 'triggered', 'expired'
  expiresAt: timestamp("expires_at"), // When this pick expires
  createdAt: timestamp("created_at").notNull().defaultNow(),
}, (table) => ({
  strategyIdIdx: index("strategy_stock_picks_strategy_id_idx").on(table.strategyId),
  symbolIdx: index("strategy_stock_picks_symbol_idx").on(table.symbol),
  statusIdx: index("strategy_stock_picks_status_idx").on(table.status),
  createdAtIdx: index("strategy_stock_picks_created_at_idx").on(table.createdAt),
}));

// Zod schemas for algorithmic strategies
export const insertAlgorithmicStrategySchema = createInsertSchema(algorithmicStrategies).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export type InsertStrategyStockPick = z.infer<typeof insertStrategyStockPickSchema>;
export type StrategyStockPick = typeof strategyStockPicks.$inferSelect;
